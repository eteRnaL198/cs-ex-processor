...
Done:	addiu	$t5,	$zero,	100
	...
	addi	$t0,	$zero,	101		
TAtest:	addi	$t0,	$t0,	-1		# 配列の中身が降順ソートされていたか判定
	beq	$t0,	$zero,	TAt		# 最後まであっていたらブザーはならない
	nop
	lw	$t1,	0($s1)			# 配列のロード
	beq	$t0,	$t1	TAtest		# 真の値と配列を比較
	nop
	addi	$s1,	$s1,	4		# 次の配列の要素へ移動
	addi 	$t5,	$zero,	300		# 間違えていたら高いブザー
	sw	$t5,	12($s3)			
	j	TAstop
	nop
TAt:	sw	$t5,	12($s3)			# あっていたら低いブザー
TAstop:	li	$v0,	1
	syscall					# 一時停止
